 ...
 *
 * Canonical copy lives in vaporware/src/startup.s.
 * Apps reference it via %VAPORWARE%\src\startup.s in their build scripts.
 * Do NOT keep per-app copies; they will diverge.
 */
...
/* ---- Stack ---- */
    .section .stack, "w"
    .align 3
    .equ Stack_Size, 0x800  /* 2 KB stack out of 8 KB SRAM */
    .space Stack_Size
_estack:
...
Reset_Handler:
    /* Copy .data section from flash load address to SRAM */
    ldr     r0, =_sdata
    ldr     r1, =_edata
    ldr     r2, =_sidata
    movs    r3, #0
...
bss_loop_check:
    ...
    /* Jump to main (provided by app.c framework) */
    bl      main
    b       .

/* ---- Default weak exception/interrupt handlers ---- */
    .weak   NMI_Handler
